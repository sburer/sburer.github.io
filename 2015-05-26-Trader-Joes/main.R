...
if(!file.exists("stores.Rdata")) {
    ...
    # Create the "logical OR" list of state abbreviations
    ...
    # were broken across two lines. Find those matches which require the
    # preceding line. Each address should start with a number
    ...
    # messing up Google's ability to geocode the address
...
# Find those stores that haven't been geocoded yet
...
# Geocode the actual DC locations that I have found
...
# Add line segments between stores and centers

p <- p + geom_segment(aes(x = lon, y = lat, xend = c.lon, yend = c.lat),
    data = subdf)

# Add proposed DC locations

p <- p + geom_point(aes(x = c.lon, y = c.lat), data = centers,
    color = I("red"), size = 4)

# Add true DC locations

p <- p + geom_point(aes(x = lon, y = lat), data = DCs,
    color = I("green"), size = 4)
